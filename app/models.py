...
class User(AbstractUser):
    ...
    def register_user(cls, data):
        """
        Create a user by mapping serializer data to model fields dynamically.
        """
        contact_value = data.get('email_or_phone_number') or data.get('contact_info')
        contact_method = data.get('contact_method')

        # Start with any explicitly provided fields
        field_map = {
            'username': 'username',
            'name': 'name',
            'role': 'role',
        ...
        for source_key, target_key in field_map.items():
            ...
            if value:
                user_payload[target_key] = value

        # Derive email/phone if only the generic contact field is provided
        if contact_value:
            if contact_method == 'EMAIL' or ('@' in contact_value and 'email' not in user_payload):
                user_payload.setdefault('email', contact_value)
                user_payload.setdefault('email_verified', True)
            else:
                user_payload.setdefault('phone_number', contact_value)
                user_payload.setdefault('phone_number_verified', True)

        # Ensure username is always set
        user_payload.setdefault('username', user_payload.get('email') or user_payload.get('phone_number'))

        password = data.get('password')
        if not password:
            raise ValueError("Password is required to register a user")

        # Remove any empty values before persisting
        cleaned_payload = {key: value for key, value in user_payload.items() if value}

        return cls.objects.create_user(password=password, **cleaned_payload)
...
class Ride(models.Model):
    customer = models.ForeignKey(User, on_delete=models.CASCADE, related_name='rides_requested')
    driver = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='rides_accepted')
    ...
    fare = models.IntegerField(default=0)  # Ride fare in smallest currency unit
    estimated_fare = models.IntegerField(null=True, blank=True)  # Estimated fare before ride
    distance = models.FloatField(null=True, blank=True)  # Distance in kilometers
    estimated_distance = models.FloatField(null=True, blank=True)  # Estimated distance
    estimated_duration = models.IntegerField(null=True, blank=True)  # Estimated duration in minutes
    status = models.CharField(max_length=20, choices=RideStatus.choices, default=RideStatus.PENDING)
    requested_at = models.DateTimeField(auto_now_add=True)
    accepted_at = models.DateTimeField(null=True, blank=True)
    started_at = models.DateTimeField(null=True, blank=True)
    completed_at = models.DateTimeField(null=True, blank=True)
    cancelled_at = models.DateTimeField(null=True, blank=True)
    cancelled_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='rides_cancelled')
    cancel_reason = models.CharField(max_length=255, null=True, blank=True)
    ...
    def formatted_requested_at(self):
        """Return the requested_at date as a formatted string."""
        return self.requested_at.strftime("%Y-%m-%d %H:%M:%S") if self.requested_at else ""

    @property
    def formatted_accepted_at(self):
        """Return the accepted_at date as a formatted string."""
        return self.accepted_at.strftime("%Y-%m-%d %H:%M:%S") if self.accepted_at else ""

    @property
    def formatted_completed_at(self):
        """Return the completed_at date as a formatted string."""
        return self.completed_at.strftime("%Y-%m-%d %H:%M:%S") if self.completed_at else ""

    def save(self, *args, **kwargs):
        # Auto-update timestamps based on status changes
        if self.status == RideStatus.ACCEPTED and not self.accepted_at:
            self.accepted_at = timezone.now()
        elif self.status == RideStatus.IN_PROGRESS and not self.started_at:
            self.started_at = timezone.now()
        elif self.status == RideStatus.COMPLETED and not self.completed_at:
            self.completed_at = timezone.now()
        elif self.status == RideStatus.CANCELLED and not self.cancelled_at:
            self.cancelled_at = timezone.now()
        super().save(*args, **kwargs)

    def __str__(self):
        return f"<Ride(id={self.id}, customer_id={self.customer.id}, status={self.status})>"